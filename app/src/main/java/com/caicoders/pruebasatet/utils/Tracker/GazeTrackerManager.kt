...
class GazeTrackerManager private constructor(context: Context) {
    ...
    fun getGazeTrackerState(): GazeTracker? {
        return gazeTracker
    }

    // TODO: change licence key
    private val SEESO_LICENSE_KEY = "API password"
    ...
    companion object {
        ...
        fun makeNewInstance(context: Context): GazeTrackerManager? {
            ...
            Log.d(
                "PutoGaze",
                "El puto gaze es ${instance?.getGazeTrackerState()} en makeNewInstance"
            )
            return instance
        }
    }
    ...
    fun isTracking(): Boolean {
        ...
        Log.d("PutoGaze", "El puto gaze es ${getGazeTrackerState()} en isTracking")
    }

    fun isCalibrating(): Boolean {
        Log.d("PutoGaze", "El puto gaze es ${getGazeTrackerState()} en isCalibrating")
    ...
    fun initGazeTracker(callback: InitializationCallback, isInitWithUserOption: Boolean) {
        ...
        // add mContext to context


        Log.d("GAZE TRACKER", mContext.get().toString())
    ...
    fun startGazeTracking(): Boolean {
        try {
            ...
            Log.d("PutoGaze", "El puto error es ${e} en startGazeTracking")
        }
        return false
    }
    ...
    private val initializationCallback: InitializationCallback = object : InitializationCallback {
        override fun onInitialized(tracker: GazeTracker?, error: InitializationErrorType?) {
            ...
            gazeTracker = tracker
            Log.d("PutoGaze", "El puto gaze es ${gazeTracker} en initializationCallback")
            Log.d("PutoGaze", "El puto tracker es ${tracker} en initializationCallback")
